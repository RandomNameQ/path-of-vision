...
namespace Poe_show_buff
{
    ...
    {
        // TODO сделать массив точек, где хранятся бафы, сразу получать контент и анализировать, если нет, то завершаем
        ...
        private ListBuffs _listBuffs = new ListBuffs();
        private DetectTargetBuffs _detectTargetBuffs;

        public MakeScreenShot()
        {
            
            DebuffPixels();
            _listBuffs = Program.ReturnBuffs();
            _detectTargetBuffs = new DetectTargetBuffs(_listBuffs.ListAllBuffs);
        ...
        public void MakeShoot()
        {
            ...
            // Clear the list of images and pixels
            ImagesAndPixels.Clear();

            // Define the game bounds as a fraction of the primary screen bounds
            ...
            for (int x = disstanceChanger; x < gameBounds.Width; x += widthImage)
            {
                ...
                // Store the image and pixels in the list
                /*ImagesAndPixels.Add(smallPicture, pixels);*/

                // Save the small picture to a file
